...
public class BossScript : MonoBehaviour
{
    ...
    public CanvasGroup ResultUICanvas;
    ...
    void Start()
    {
        ...
        ResultReturnButton.onClick.AddListener(ResultReturnClick);
    }

    public void LinkBossNode(Button b)
    {
        b.onClick.AddListener(BossNodeClicked);
    }

    private void BossNodeClicked()
    {
        disable = false;
        enemyHealth = 30;
    ...
    {
        StartCoroutine(ResultUIFadeOut());
    }
    ...
    IEnumerator ResultFadeIn()
    {
        ResultUICanvas.alpha = 0;
        ResultUICanvas.gameObject.SetActive(true);
        while (ResultUICanvas.alpha < 1)
        {
            ResultUICanvas.alpha += Time.deltaTime;
            yield return null;
        }
    }

    IEnumerator ResultUIFadeOut()
    {
        while (ResultUICanvas.alpha > 0)
        {
            ResultUICanvas.alpha -= Time.deltaTime;
            yield return null;
        }
        ResultUICanvas.gameObject.SetActive(false);
        gameObject.GetComponent<Return>().endStage();
    }
